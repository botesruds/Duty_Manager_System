...
const variantClass: Record<Variant, string> = {
  primary:
    'bg-brand-700 hover:bg-brand-800 text-white disabled:bg-slate-300 disabled:text-slate-500',
  secondary:
...
export function Button({ variant = 'primary', className = '', ...rest }: ButtonProps) {
  return (
    <button
      className={`inline-flex items-center justify-center rounded-lg px-3 py-2 text-sm font-medium shadow-sm transition focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-1 disabled:cursor-not-allowed ${variantClass[variant]} ${className}`}
      {...rest}
    />
  )
}
